...
EXPECTED_HEADERS = {
    "RECORD ID": "record_id", "BADGE NUMBER": "badge_number", "RANK": "rank",
    "LAST NAME": "last_name", "FIRST NAME": "first_name", "MIDDLE NAME": "middle_name",
    "SUFFIX": "suffix", "CAMP": "camp", "OFFICE": "office", "GENDER": "gender",
    "CLASSIFICATION": "classification", "TYPE": "personnel_type",
    "DUPLICATE STATUS": "duplicate_status", "DUPLICATE TYPE": "duplicate_type",
...
ON CONFLICT(badge_number) DO UPDATE SET
    record_id=excluded.record_id, rank=excluded.rank, last_name=excluded.last_name,
    first_name=excluded.first_name, middle_name=excluded.middle_name, suffix=excluded.suffix,
    camp=excluded.camp, office=excluded.office, gender=excluded.gender,
    classification=excluded.classification, personnel_type=excluded.personnel_type,
    duplicate_status=excluded.duplicate_status, duplicate_type=excluded.duplicate_type,
    created_at=excluded.created_at, updated_at=excluded.updated_at,
    source_order=excluded.source_order, drive_file_id=excluded.drive_file_id;
"""
    ...
    with connect(db_path) as connection:
        for source_order, values in enumerate(rows, start=1):
            ...
            for index, field in column_map.items():
                ...
                value = values[index]
                if field == "badge_number": record[field] = normalize_badge(value)
                elif field == "drive_file_id": record[field] = normalize_drive_file_id(value)
                else: record[field] = normalize(value)
